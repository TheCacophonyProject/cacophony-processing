...
from thermal_processing import calculate_tag, FALSE_POSITIVE, UNIDENTIFIED
...
class TestTagCalculations:
    ...
    def test_one_track(self):
        assert calculate_tag([{"label": "rat", "confidence": 0.9}]) == ("rat", 0.9)

    def test_one_track_low_confidence(self):
        assert calculate_tag([{"label": "rat", "confidence": 0.5}]) == (
            UNIDENTIFIED,
            0.85,
        )

    def test_false_positive_low_confidence(self):
        assert calculate_tag([{"label": "false-positive", "confidence": 0.39}]) == (
            FALSE_POSITIVE,
            0.85,
        )
    ...
    def test_multi_track_same_animal(self):
        assert calculate_tag(
            [{"label": "rat", "confidence": 0.5}, {"label": "rat", "confidence": 0.9}]
        ) == ("rat", 0.9)
